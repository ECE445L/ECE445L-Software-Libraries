...
char    eid[32]           = "your_eid_goes_here";       // Your EID goes here
char    ssid[32]          = "EE-IOT-Platform-03";       // WiFi Access Point in the 445L Lab
char    pass[32]          = "g!TyA>hR2JTy";
char    mqtt_broker[32]   = "10.159.177.113";           // MQTT Broker in EER 5.826
...
{ 
  ...
  UART2_OutString("Waiting for RDY flag from ESP\r\n");
  #endif
  ...
  UART5_OutChar('\n');            // Send NL to indicate EOT   
    ...
    UART2_OutString(eid);    
    UART2_OutChar(',');
    UART2_OutString(ssid);
    UART2_OutChar(',');
    UART2_OutString(pass);
    UART2_OutChar(',');
    UART2_OutString(mqtt_broker);
    UART2_OutChar(',');
    UART2_OutString(mqtt_port);
    UART2_OutChar(',');
    UART2_OutChar('\n');      
    #endif
  ...
  while ((RDY) | ((UART5_FR_R & UART5_FR_RXFE) ==0)) { 
    ...
    #ifdef DEBUG1
        UART2_OutChar(inchar);                        // Output received character
    #endif
    }
  ...
  #ifdef DEBUG1
  UART2_OutString("Exiting WiFI_Setup routine\r\n");
